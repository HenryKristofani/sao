...
@section('content')
    ...
    <div class="content p-4" style="margin-top: 60px;">
        ...
        <div class="card">
            <div class="card-body">
                <form action="{{ route('pabrik.po-jual.update', $penjualan->id_penjualan) }}" method="POST" id="poForm">
                    @csrf
                    @method('PUT')
                    <div class="mb-3">
                        <label for="id_detail_sales" class="form-label">ID Detail Penjualan</label>
                        <input type="text" class="form-control bg-light" id="id_detail_sales" value="{{ $penjualan->id_penjualan }}" readonly>
                    </div>

                    <div class="mb-3">
                        <label for="id_sales" class="form-label">ID Penjualan</label>
                        <input type="text" class="form-control bg-light" id="id_sales" value="{{ $penjualan->id_penjualan }}" readonly>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="date" class="form-control bg-light" name="sale_date" id="sale_date" value="{{ $penjualan->tanggal_penjualan }}" readonly>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="employee_id" class="form-select" id="employee_id" required>
                            ...
                            @foreach($karyawan as $k)
                                <option value="{{ $k->id_karyawan }}" {{ $penjualan->id_karyawan == $k->id_karyawan ? 'selected' : '' }}>{{ $k->nama_karyawan }}</option>
                            @endforeach
                        </select>
                        @error('employee_id')
                            <div class="text-danger">{{ $message }}</div>
                                    ...
                                    <div class="col-md-4 mb-2">
                                        ...
                                        <select name="items[{{ $index }}][item_id]" class="form-select item-select" required>
                                            ...
                                            @foreach($items as $item)
                                                <option value="{{ $item->id_item }}" data-price="{{ $item->harga_per_item }}" {{ $detail->id_item == $item->id_item ? 'selected' : '' }}>{{ $item->nama_item }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-md-3 mb-2">
                                        <label class="form-label">Jumlah</label>
                                        <input type="number" name="items[{{ $index }}][quantity]" class="form-control item-quantity" min="1" value="{{ $detail->jumlah_jual }}" required>
                                    </div>
                                    <div class="col-md-3 mb-2">
                                        <label class="form-label">Harga Satuan</label>
                                        <input type="number" name="items[{{ $index }}][unit_price]" class="form-control item-price" value="{{ $detail->harga_jual_satuan }}" readonly>
                                    </div>
                                    <div class="col-md-2 mb-2">
                                        <label class="form-label">Subtotal</label>
                                        <input type="text" class="form-control item-subtotal" value="Rp {{ number_format($detail->subtotal_harga, 0, ',', '.') }}" readonly>
                                    </div>
                                </div>
                                <div class="row mt-2">
                                    <div class="col-12">
                                        <button type="button" class="btn btn-sm btn-danger remove-item" {{ count($detailPenjualan) <= 1 ? 'style=display:none;' : '' }}>Hapus Item</button>
                                    </div>
                                </div>
                            </div>
                        @endforeach
                            ...
                            <div class="card">
                                <div class="card-body">
                                    <div class="d-flex justify-content-between mb-2">
                                        <strong>Total:</strong>
                                        <span id="grand-total">Rp {{ number_format($penjualan->total_harga_penjualan, 0, ',', '.') }}</span>
                                        <input type="hidden" name="total_price" id="total-price-input" value="{{ $penjualan->total_harga_penjualan }}">
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <script>
        ...
        let itemCounter = {{ count($detailPenjualan) }};
        ...
        function calculateSubtotal(row) {
            ...
            // Format ke format rupiah dengan pemisah ribuan
            const formattedSubtotal = 'Rp ' + subtotal.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".");
